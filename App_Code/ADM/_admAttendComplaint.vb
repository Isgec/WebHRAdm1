...
Namespace SIS.ADM
	...
	Partial Public Class admAttendComplaint
		...
		Public Shared Function GetNewRecord() As SIS.ADM.admAttendComplaint
			Dim Results As SIS.ADM.admAttendComplaint = Nothing
			Results = New SIS.ADM.admAttendComplaint()
			Return Results
		End Function
		<DataObjectMethod(DataObjectMethodType.Select)> _
		Public Shared Function GetByID(ByVal CallID As Int32) As SIS.ADM.admAttendComplaint
			Dim Results As SIS.ADM.admAttendComplaint = Nothing
			Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetConnectionString())
				Using Cmd As SqlCommand = Con.CreateCommand()
					Cmd.CommandType = CommandType.StoredProcedure
					Cmd.CommandText = "spadmAttendComplaintSelectByID"
					SIS.SYS.SQLDatabase.DBCommon.AddDBParameter(Cmd, "@CallID", SqlDbType.Int, CallID.ToString.Length, CallID)
					Con.Open()
					Dim Reader As SqlDataReader = Cmd.ExecuteReader()
					If Reader.Read() Then
						Results = New SIS.ADM.admAttendComplaint(Reader)
					End If
					Reader.Close()
				End Using
			End Using
		...
		<DataObjectMethod(DataObjectMethodType.Select)> _
		Public Shared Function GetByID(ByVal CallID As Int32, ByVal Filter_EndUserID As String, ByVal Filter_CallTypeID As String, ByVal Filter_CallStatusID As String) As SIS.ADM.admAttendComplaint
			Return GetByID(CallID)
		End Function
		<DataObjectMethod(DataObjectMethodType.Select)> _
		Public Shared Function SelectList(ByVal startRowIndex As Integer, ByVal maximumRows As Integer, ByVal orderBy As String, ByVal SearchState As Boolean, ByVal SearchText As String, ByVal EndUserID As String, ByVal CallTypeID As String, ByVal CallStatusID As String) As List(Of SIS.ADM.admAttendComplaint)
			Dim Results As List(Of SIS.ADM.admAttendComplaint) = Nothing
			Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetConnectionString())
				Using Cmd As SqlCommand = Con.CreateCommand()
					If orderBy = String.Empty Then orderBy = "LoggedOn DESC"
					Cmd.CommandType = CommandType.StoredProcedure
					If SearchState Then
						Cmd.CommandText = "spadmAttendComplaintSelectListSearch"
						SIS.SYS.SQLDatabase.DBCommon.AddDBParameter(Cmd, "@KeyWord", SqlDbType.NVarChar, 250, SearchText)
					Else
						Cmd.CommandText = "spadmAttendComplaintSelectListFilteres"
					...
					_RecordCount = -1
					Results = New List(Of SIS.ADM.admAttendComplaint)()
					Con.Open()
					Dim Reader As SqlDataReader = Cmd.ExecuteReader()
					While (Reader.Read())
						Results.Add(New SIS.ADM.admAttendComplaint(Reader))
					End While
					Reader.Close()
		...
		End Function
		'		Autocomplete Method
		Public Shared Function SelectadmAttendComplaintAutoCompleteList(ByVal Prefix As String, ByVal count As Integer, ByVal contextKey As String) As String()
			Dim Results As List(Of String) = Nothing
			Dim aVal() As String = contextKey.Split("|".ToCharArray)
			Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetConnectionString())
				Using Cmd As SqlCommand = Con.CreateCommand()
					Cmd.CommandType = CommandType.StoredProcedure
					Cmd.CommandText = "spadmAttendComplaintAutoCompleteList"
					SIS.SYS.SQLDatabase.DBCommon.AddDBParameter(Cmd, "@AssignedTo", SqlDbType.NVarChar, 8, Global.System.Web.HttpContext.Current.Session("LoginID"))
					...
					End If
					While (Reader.Read())
						Dim Tmp As SIS.ADM.admAttendComplaint = New SIS.ADM.admAttendComplaint(Reader)
