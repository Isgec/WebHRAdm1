...
Public Class RPT_admRegisterComplaint
	...
	Public Overrides Sub ProcessReport()
		...
		Dim oCalls As List(Of SIS.ADM.admRegisterComplaint) = Nothing
		Dim LastValue As String = ""

		Do While True
			oCalls = SIS.ADM.admRegisterComplaint.SelectList(dbPage * dbSize, dbSize, dbOrder, False, "", EndUserID, CallTypeID, AssignedTo, CallStatusID)
			If oCalls.Count <= 0 Then
				Exit Do
			End If
			dbPage += 1

			For Each det As SIS.ADM.admRegisterComplaint In oCalls
				sn += 1

				Print("<tr>")
				Print("<td style=""font-size:8px;text-align:right"">" & sn.ToString & "</td>")
				Print("<td style=""font-size:8px;text-align:right"">" & det.CallID & "</td>")
				Print("<td style=""font-size:8px;text-align:left"">[" & det.EndUserID & "] " & det.FK_ADM_Complaints_HRM_Employees.EmployeeName & "</td>")
				Print("<td style=""font-size:8px;text-align:left"">" & det.FK_ADM_Complaints_ADM_CallTypes.Description & "</td>")
				Print("<td style=""font-size:8px;text-align:left"">" & det.Description & "</td>")
				Print("<td style=""font-size:8px;text-align:left"">" & det.FK_ADM_Complaints_ADM_Users1.UserFullName & "</td>")
				...
				Print("<td style=""font-size:8px;text-align:left"">" & det.FK_ADM_Complaints_ADM_Users.UserFullName & "</td>")
